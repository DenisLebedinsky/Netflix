    ...
    private func configureHeroHeaderView(){
        APICaller.shared.getTrendingMovies { [weak self] result in
            ...
            case .success(let titles):
                ...
                let title = selectedTitle?.original_title ?? selectedTitle?.original_name ?? ""
    
                self?.headerView?.configure(with: TitleViewModel(titleName: title, posterURL: selectedTitle?.poster_path ?? ""))
...
extension HomeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, willDisplayHeaderView view: UIView, forSection section: Int) {
        ...
        header.textLabel?.text = header.textLabel?.text?.capitalizeFirsLatter()
    }
